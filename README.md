...
현재 상태: **Phase 0–7 (편집기 4차) 완료** — 타입 시스템 · 레지스트리 · 컴파일러(G1/G2) · decompile 왕복 ·
...
Node Library에서 노드를 누르거나 캔버스로 끌어다 놓아 추가하고, History 항목을 누르면
그 시점으로 되감는다(프레임워크 없이 표준 라이브러리만 쓴다).
실물 백본은 `backbone: hf:<모델 경로>` 한 줄로 들어온다 — 어댑터가 `config.json`만 읽어
예산을 답하므로 가중치 없이도 G4가 돈다. 남은 것은 모델 id 결정과 다운로드,
그리고 편집기의 Parameter Recipe 패널과 라이브 실행 갱신.
