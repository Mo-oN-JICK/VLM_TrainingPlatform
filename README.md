...
현재 상태: **Phase 0–7 (편집기 7차) 완료** — 타입 시스템 · 레지스트리 · 컴파일러(G1/G2) · decompile 왕복 ·
...
Run을 누르면 편집기가 `vlmt run`을 그대로 하위 프로세스로 띄우고, 그 실행이 남기는
진행 파일을 폴링해 카드에 상태를 칠한다 — UI 전용 실행 경로는 없다
