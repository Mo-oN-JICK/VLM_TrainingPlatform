...
**이 시점에 없던 것**(이후 2~7차에서 전부 붙었다): 편집, 라이브 갱신, History 되감기.
편집기는 이 뷰어 위에 얹었다.
...
### Phase 7 (7차) — 세션을 넘는 History ✅

- [x] 저널(`edit_history.jsonl`)은 **사람이 읽는 diff**로 그대로 두고, 되감기에 필요한 스펙 본문은
      옆의 `edit_history/`에 **내용 주소**로 따로 둔다. 둘을 한 파일에 섞으면 저널을 읽을 수 없게 된다
- [x] 내용 주소라 같은 상태로 돌아와도 파일이 늘지 않는다. 여는 순간 저널이 더 이상 가리키지 않는
      본문은 지운다 — 저장소는 저널을 따라간다
- [x] 열 때 저널의 마지막 200개를 되짚어 시점을 되살린다. **본문이 남아 있는 시점만** 되살린다:
      되감을 수 없는 항목이 목록에 섞이면 누를 수 있는 것과 없는 것이 뒤섞여 History가 신뢰를 잃는다
- [x] 디스크의 스펙이 마지막 시점과 같으면 `열기`를 겹쳐 적지 않는다. 손으로 YAML을 고쳐 두었다면
      다르므로 지금 상태가 새 시점으로 붙는다
- [x] 되살린 시점은 표식이 붙고(흐린 라벨 + 점), 툴팁에 날짜가 나온다. 저널의 `at`이 날짜까지
      담도록 바뀌었다 — 시:분:초만으로는 세션을 넘을 수 없다
- [x] 검증 5개 추가 (`tests/test_editor.py` 61개, 전체 **191 passed**)

**Phase 7의 3b가 이것으로 끝났다.** UI만으로 그래프를 만들고, 값을 고치고, 레시피를 씌우고,
저장하고, 실행하고, 어제의 시점으로 되감을 수 있다.
...
- 완료 조건: UI만으로 그래프 하나를 처음부터 만들어 CLI로 실행한다 — **충족**(4b에서 마무리).
  한때 YAML로만 되던 네 가지(새 프로젝트 생성·`sample_space`·`materialize` 경계·`runtime_profile`)가
  모두 UI로 들어왔다. 남은 YAML은 Trainer 설정(`trainer.yaml`) 하나다.
...
## 4c. 사람이 읽을 수 있는 화면 + 두 번째 도메인 ✅ (2026-09-11)

사용자 지적: "사람이 이해하기 너무 어려운 형태다. Flow.png 정도의 노드와 배선을 예상했다."
카드 25장이었다. **캡슐화 기계(Procedure)는 이미 있었는데 캔버스가 그걸 버리고 있었다.**

- [x] **캔버스가 Procedure를 접는다.** 상자 안에서 끝나는 배선은 사라지고, 밖으로 드나드는
      배선은 노출 포트로 옮겨 붙는다. 카드의 `▸N` 으로 펼치고 접는다.
      **접기는 보는 방식일 뿐** — History에도 안 남고 spec_hash도 같다. 게이트는 언제나 펼쳐진 그래프를 본다
- [x] 우측 패널도 함께 접힌다 — 접힌 상자는 **노출 파라미터만** 보인다.
      캔버스만 접고 패널을 그대로 두면 복잡도가 옮겨갈 뿐이다
- [x] 접힌 상자의 상태는 안쪽의 **최악**을 따른다. 상자가 초록인데 안이 빨간 것이 제일 나쁜 화면이다
- [x] 접은 뒤 비는 레인을 촘촘히 다시 매긴다. 헤더가 `상자 9 · 노드 12`를 둘 다 적는다

- [x] **공용 Procedure 3개** (`solutions/procedures/`): `image_prep_resize`, `prompt_build`,
      `answer_build`. 로더가 4단계 위까지 올라가며 찾으므로 Solution 사이에서 공유된다
- [x] **두 번째 도메인 `vlm_parts`** — 이미지 한 장 + 라벨 하나. 시계열·전문가 모델·지식문서 없음.
      compile → dryrun → budget → run → materialize → train 전부 완주.
      **노드 12개가 상자 9개로 그려진다**
- [x] 이미지 준비 두 갈래(`image_prep_resize` / `expert_crop`)가 **같은 포트 이름과 타입**을 내서
      배선 하나로 갈아 끼운다. 정답 모양은 그래프가 아니라 **스키마 파일**이 정한다

**두 번째 도메인이 드러낸 것 둘**

1. **Procedure의 노출 입력이 안쪽 한 포트에만 갈 수 있었다.** 스키마는 정답을 쓰는 노드와
   검사하는 노드 **둘 다** 필요하다. 팬아웃을 허용했다(팬인과 다르다). 출력은 여전히 하나다
2. **스키마 단계의 `evidence` 기본값이 `required`였다.** 근거 문장을 강제하는 것은 ECG/Triad
   한 도메인의 규약인데 모든 정답의 기본값이 돼 있었다. 가장 단순한 스키마(라벨 하나)가
...
## 9. 실물 모델로 한 바퀴 — 진행 중 (2026-09-22)

`tiny-vlm` 으로는 손실이 내려가는 것까지 봤다. 그것으로는 설계가 맞는지 알 수 없다 —
진짜 VLM 이 진짜 이미지를 보고 사람이 읽을 수 있는 답을 내놓아야 한 바퀴가 돈 것이다.
계획서: `C:/Users/wnsgu/.claude/plans/real-model-loop.md` (9단계, 약 9일).

### 0단계 ✅ — 실측으로 확정한 것

`Qwen/Qwen2-VL-2B-Instruct` 가중치를 받아 프로세서만 돌렸다. 학습은 하지 않았다.

```
image_token       <|image_pad|>   (id 151655)
정식 자리표시자    <|vision_start|><|image_pad|><|vision_end|>
                  (151652 / 151655 / 151653)
image_grid_thw    [1, 32, 32]  ->  펼쳐진 이미지 토큰 256개
<image>           vocab 에 없음 (None)
```

**`<image>` 는 예외 없이 "통과" 한다. 이것이 함정이다.** 프로세서가 거부하지 않고 조용히
지나간다 — vocab 에 없으니 평범한 글자 몇 개로 토큰화되고, `<|image_pad|>` 가 하나도
없으니 **이미지와 텍스트가 연결되지 않은 채 학습이 돈다.** 손실은 내려가는데 모델은
이미지를 보지 않는다. 계획의 4단계(백본이 자리표시자를 번역)는 선택이 아니라 필수다.

덤: 토큰/타일이 **256개**로 실측됐고 `trainer_qwen2vl.yaml` 의 `tokens_per_tile: 256` 과
일치한다. G4 예산이 맞는 값을 쓰고 있었다.

- [x] `vlmt backbones --fetch <ref>` — 가중치는 **시켜야** 받는다. `--add` 는 여전히
      config.json 만 읽는다(예산 게이트가 부르는 길이라 그 자리에서 GB 가 떨어지면 안 된다)
- [x] `torchvision` 필요. Qwen2-VL 프로세서가 비디오 처리기를 함께 끌고 온다
      (`0.29.0+cu130`)
- [x] `.claude/` 를 저장소에서 뺐다 — 전역 설정으로 옮겼으므로 프로젝트에 있을 이유가 없다

### 1단계 ✅ — 노드 이름을 Process 용어로

| 전 | 후 |
|---|---|
| 정답 스키마 정의 | 답변 형식 정하기 |
| 정답 텍스트 생성 | 학습용 정답 답변 생성 |
| 정답 스키마 검증 | 답변 형식 검사 |
| 정답 누설 검사 | 문제에 답이 섞였는지 검사 |
| 학습 샘플 조립 | 학습 데이터 맵핑 |
| 이미지 슬롯 삽입 | 학습 이미지 맵핑 |

`이미지 리사이즈` · `이미지 좌표계 변환` · `단일값 -> 리스트` 는 **그대로 둔다** —
업계에서 쓰는 말이라 바꾸면 검색도 안 되고 아는 사람에게 낯설어진다.

### 남은 단계

- [ ] **2단계 — Output 규칙 변경** ⚠ 가장 위험. 지금 Output 은 출력 포트를 가질 수 없다.
      `모델 학습` 에서 `모델 추론` 으로 선을 이으려면 이 규칙이 바뀌어야 한다.
      `io.dataset_export` · 물질화 경계 · 위상 정렬이 "Output 은 끝점" 을 전제할 수 있다.
      **후퇴 지점**: 깨지면 추론 노드가 경로를 파라미터로 받는 방식으로 물러난다
- [ ] 3단계 — `모델 추론` 노드 (`PROCESSING` + `external_call`). `tiny-vlm` 으로 먼저 완성
- [ ] 4단계 — 백본이 자리표시자를 번역. 그래프는 계속 `<image>` 만 안다
- [ ] 5단계 — `실행` 파이프라인. CLI 에 먼저(`vlmt pipeline`), 앱 버튼은 그 다음
- [ ] 6단계 — `backbone:` 한 줄로 Qwen2-VL 교체. **새 코드가 없어야 한다**
- [ ] 7단계 — `모델 Export`(합친 모델 + contract) · 문서

### 확정된 결정

목적은 **확인**이지 제품이 아니다. 끝점은 "검증 10장에 스키마대로 답한다" 이고,
"loss 가 내려갔다" 로는 부족하다(`tiny-vlm` 에서 이미 봤다).

- 자동 채점은 **하지 않는다** — 라벨 잡음이 열에 두셋이라 숫자를 믿게 되는 게 더 나쁘다
- `color_space` 타입 구멍은 **미룬다** — 실재하지만 지금 아무것도 안 깨뜨린다
- 버튼은 `실행` 하나 + 단계 표시. **그래프에 있는 데까지** 돈다
- 중단 후 재개는 **물어본다**
...
`C:\Users\wnsgu\.claude\plans\snappy-drifting-dawn.md`.
...
| | 내용 | 커밋 |
|---|---|---|
| 0 | PySide6 6.11.2 · `pyproject` 의 선택 의존성 `[app]` | `1261862` |
| 1 | `ui/layout.py` 추출 — 배치·상태 계산 341줄 | `1261862` |
...
- **이벤트 핸들러 안에서 `scene.clear()` 를 부르면 죽는다(세그폴트).** Qt 가 지금
  ...
  루프 안에서 사라진다. 죽는 곳이 C++ 이라 **파이썬 트레이스백이 안 나온다**.
  → 변경을 전부 `QTimer.singleShot(0, ...)` 로 미루고, 메뉴는 닫힌 뒤에 판단한다
- **`QT_QPA_PLATFORM=offscreen` 은 폰트가 0종이다.** 한글이 전부 네모로 나온다.
  구조 검사에만 쓰고, 렌더링 확인은 `show()` 없이 `view.grab()` — 창이 뜨지 않으면서
  실제 폰트로 그려진다
- 카드는 배치 좌표보다 입력 칩 높이(`CHIP_H` 30px)만큼 아래에 그려진다. 옮긴 자리를
  그대로 저장하면 움직일 때마다 밀려난다 → `NodeCard.top_h` 를 빼서 저장
- `Editor.open()` 은 `edit_history.jsonl` 과 `layout.yaml` 을 **쓴다**. 테스트는
...
"자동 캐스팅 없이 명시적 `adapt.*` 로만" 이라는 전제에서 보면 구멍이다. 미해결.
