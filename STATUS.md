...
- 완료 조건: UI만으로 그래프 하나를 처음부터 만들어 CLI로 실행한다 — **부분 충족**.
  3b의 항목은 전부 끝났지만 "UI만으로"는 과장이었다. 아직 YAML을 직접 만져야 하는 것:
  **새 프로젝트 생성**(`edit`는 기존 project.yaml을 연다), **`sample_space`**(데이터 소스·필터·split),
  **`materialize`** 경계, **`runtime_profile`**. 노드·배선·파라미터·레시피는 UI에서 끝난다.
